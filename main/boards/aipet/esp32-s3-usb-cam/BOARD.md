...
当前代码：WiFi + 音频 + 双眼 C1 + 两颗 WS2812。**P1**：`self.eye.set_emotion` 同轮改灯；`self.eye.set_blink_profile` 可调眨眼间隔。**当前嵌入皮肤=巨蟹座**（`image/zodiac/cancer`，128×160）。天蝎已验素材并烧过；双鱼已转好未嵌入。无运行时换皮肤。CN2 跟随代码仍在，舵机/触摸硬件未接。UART1 收 K230 JSON。4G 起板探测由 `BOARD_ENABLE_4G_TEST` 控制，**当前为 0**：不开 UART2、不拉 `4G_PWR`、不发 AT。
...
| 4 | KEY1 | 用户键 SW3 | 仅宏 |
| 5 / 6 / 7 | IO4/5/6_TOUCH* | 触摸 CN1（丝印名不是 GPIO 号） | 仅宏 |
| 8 | SE_PWMO | 舵机 CN2 信号（MG90S，50Hz LEDC T1/CH1） | 已开；默认跟 K230 `tracking.dx` |
...
| 18 | 4G_PWR | TPS562200 EN，高电平出 +4V VBAT | `BOARD_ENABLE_4G_TEST=0` 时不驱动，靠 R66 下拉关 |
...
| 47 | IO47_U2RXD / 4G_UART0_TXD | UART2 RX ← ML307 UART0_TXD | 仅 `BOARD_ENABLE_4G_TEST=1` 时开 |
| 48 | IO48_U2TXD / 4G_UART0_RXD | UART2 TX → ML307 UART0_RXD | 仅 `BOARD_ENABLE_4G_TEST=1` 时开 |
