...
当前代码：WiFi + 音频 + 双眼 C1 + 两颗 WS2812。**P1**：`self.eye.set_emotion` 同轮改灯；`self.eye.set_blink_profile` 可调眨眼间隔。**当前嵌入皮肤=巨蟹座**（`image/zodiac/cancer`，128×160）。天蝎已验素材并烧过；双鱼已转好未嵌入。无运行时换皮肤。CN2 跟随代码仍在，舵机/触摸硬件未接。UART1 收 K230 JSON；UART2 发 ML307 AT。`4G_PWR` 已拉高。
...
| 18 | 4G_PWR | TPS562200 EN，高电平出 +4V VBAT | 已拉高 |
...
| 47 | IO47_U2RXD / 4G_UART0_TXD | UART2 RX ← ML307 UART0_TXD | 已开；无电平转换，原理图仍标 1.8V? |
| 48 | IO48_U2TXD / 4G_UART0_RXD | UART2 TX → ML307 UART0_RXD | 已开 |
