...
当前代码：WiFi + 音频 + 双眼 C1 表情（右眼原图、左眼镜像，MCP `self.eye.*`）+ 两颗 WS2812（独立 MCP `self.led.*`）。舵机、4G、K230 仅登记引脚。
...
| 10 | IO10_U1TXD | → K230 GPIO45 RX | 仅宏 |
| 11 | IO11_U1RXD | → K230 GPIO44 TX | 仅宏 |
...
| 47 | 4G_UART0_TXD | S3 RX ← ML307 UART0_TXD | 仅宏；电平待确认 |
| 48 | 4G_UART0_RXD | S3 TX → ML307 UART0_RXD | 仅宏；电平待确认 |
...
2. **4G UART 电平**：原理图仍标 `1.8V?`。S3 为 3.3V，ML307 常为 1.8V。未测通前不要拉高 `4G_PWR`。
