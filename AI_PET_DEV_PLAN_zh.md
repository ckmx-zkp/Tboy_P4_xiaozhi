...
1. 两只机器人在外出场景用低速 BLE 匿名广播互相发现。
2. 发现后由机器人主动唤醒并询问主人是否愿意打招呼。
3. 拒绝或超时立即结束；不得交换身份 token、不得向云端上报偶遇。
4. 双方主人同意后，设备通过 BLE 交换短期 `social_token`。
5. 每只设备各自经受控云通道上报 `discovery_nonce + peer_token`。
6. backend 匹配双方报告并生成本次双方要播放的内容；设备只播放分配给自己的内容并回执结束。

这不是 App/智控台配对，也不是把两个语音会话实时互桥。固件不把对方扬声器声音作为自由循环
对话输入；本地首次询问与 E11 远程主动播报也不是同一个功能。

### 固件待实现边界

- 同意前 BLE 包仅含协议版本、能力位、一次性 `discovery_nonce`，不得含 MAC、设备 UID、
  可解析身份 token 或固定跟踪标识。
- 只有双方同意确认完成后才进入 token 交换态；短期 token 不写串口日志、不持久化。
- 为扫描、询问、等待同意、交换、上报、等待内容、播放、结束/失败建立有超时的状态机；
  重启、离场、单边拒绝和重复广播均可收敛到安全终态。
- 上报和内容下行不能假设语音 WS 常驻。控制通道未拍板前仅可做 BLE 包格式、状态机和本地
  模拟测试，不接临时网络链路。
- P4 主控无内置 BLE，实际 BLE 扫描/广播需确认由 C6 `esp_hosted` 暴露能力还是另设协处理
  路径；当前 BOX-3/LCD EV Board 的 ESP32-S3 可直接做原型，但跨板型协议必须一致。

### 固件验收补充

- 同意前抓包看不到稳定设备身份；拒绝后无 token 交换和云端报告。
- 双方同意后仅交换短期 token，过期/重放/重复上报均被拒绝或幂等处理。
- 两台真机完成 backend 生成内容的受控播报，任一侧离线时另一侧能本地收尾且不死循环。
- BLE 扫描/广播不得破坏 Wi-Fi、音频 AEC、唤醒词和屏幕刷新。
